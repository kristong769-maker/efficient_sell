...
- 模块二：扫描全部可出售的 Steam 集换式卡牌，可以按各自市场最低在售价挂单，
  也可以按各自当前最高求购价优先立即成交。
- 模块三：扫描全部可出售的 CS2 武器箱，可以按各自市场最低在售价挂单，也可以
  按各自当前最高求购价优先立即成交；不会包含纪念包、胶囊等其他容器。
...
5. 可以输入每件价格并选择“买家支付总价”或“卖家实收金额”，也可在模块一选择
   ...
   无需手动输入价格。
   如需出售武器箱，可在模块三选择“以市场底价出售全部武器箱”或
   “以最高求购价出售全部武器箱”，核对每种武器箱的价格后勾选确认并出售。
...
git tag -a v1.1.2 -m "Steam 库存一键出售 v1.1.2"
git push origin master
git push origin v1.1.2
```

`.github/workflows/release.yml` 会在标签推送后自动运行测试，生成
`efficent_sell-v1.1.2.zip` 和对应的 `.sha256`，并创建 GitHub Release。程序只把
